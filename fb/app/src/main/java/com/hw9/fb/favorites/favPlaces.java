...
public class favPlaces extends Fragment {
    public static favPlaces myInstance = null;
    ViewPager pager;
    ...
    favPlaceListViewAdapter adapter;

    public static favPlaces getInstance(){
        if(myInstance!=null) return myInstance;
        myInstance = new favPlaces();
        return myInstance;
    }
    ...
    public void getUserdata(final Context ctx, JSONArray jsonArray) {
        try{
            ...
            adapter = new favPlaceListViewAdapter(ctx, R.id.listView, jsonObjects);
            if(this.view!=null){
    ...
    public void onResume() {
        super.onResume();
        getUserdata(getContext(), favMan.getFavorite("Places"));
    }
}
