...
public class favUser extends Fragment {
    public static favUser myInstance = null;
    ViewPager pager;
    ...
    favUserListViewAdapter adapter;

    public static favUser getInstance(){
        if(myInstance!=null) return myInstance;
        myInstance = new favUser();
        return myInstance;
    }
    ...
    public void getUserdata(final Context ctx, JSONArray jsonArray) {
        try{
            ...
            adapter = new favUserListViewAdapter(ctx, R.id.listView, jsonObjects);
            if(this.view!=null){
    ...
    public void onResume() {
        super.onResume();
        getUserdata(getContext(), favMan.getFavorite("Users"));
    }
}
